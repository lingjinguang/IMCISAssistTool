...
namespace IMCISAssistTool.Utils
{
    public static class PrintUtils
    {
        ...
        /// 打印图片（单张）
        /// </summary>
        public static string PrintImage(string data, string eventType, string printType, string direction, string printerName, string paperName)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(data))
                    return "要打印的数据不存在";
                Image image = (printType == "FILEPATH" || printType == "HTML") ? new Bitmap(data) : Base64ToImg(data.Replace(@"data:image/jpeg;base64,", ""));

                PrintImagePage(image, eventType, direction, printerName, paperName, setting);
            }
            catch (Exception ex)
            {
        ...
        public static string PrintImage(Image image, string eventType, string direction, string printerName, string paperName) 
        {
            try
            {
                Setting setting = FileUtils.GetSetting();
        ...
        public static void PrintImagePage(Image image, string eventType, string direction, string printerName, string paperName, Setting setting)
        {
        ...
        */
        ...
        public static string PreViewByBase64(string data, string eventType, string direction, string printerName, string paperName)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(data))
                    return "要打印的数据不存在";
                Image image = Base64ToImg(data.Replace(@"data:image/jpeg;base64,",""));
        ...
        */
        ...
        /// 打印PDF
        /// </summary>
        public static string PrintPdf(string filePath, string printType, string direction, string printerName, string paperName)
        {
            try
            {
                Setting setting = FileUtils.GetSetting();
        ...
        public static void PrintPage(object sender, PrintPageEventArgs e, Image image, Boolean hasMorePages, string direction)
        {
            PageSettings settings = e.PageSettings;
            var paperHeight = settings.PaperSize.Height;
            var paperWidth = settings.PaperSize.Width;
        ...
        /// 打印预览PDF
        /// </summary>
        public static string PreViewPdf(string filePath, string printType, string direction, string printerName, string paperName)
        {
            try
            {
                Setting setting = FileUtils.GetSetting();
                ...
                using (var pdfDocument = PdfiumViewer.PdfDocument.Load(filePath))
                {
                    ...
                    pd.PrintPage += (o, e) =>
                    {
                        PreViewPage(o, e, pdfDocument,direction, ref pages);
                    };
